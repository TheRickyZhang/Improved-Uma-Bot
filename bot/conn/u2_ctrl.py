...
from module.umamusume.constants.timing_constants import (
    TAP_EXTRA_DELAY, SCREENCAP_RETRY, RECOVERY_DELAY, SCROLL_DELAY,
)
...
class U2AndroidController(AndroidController):
    ...
    @classmethod
    def _get_adb(cls):
        if cls._adb_cmd is None:
            adb = shutil.which("adb")
            if adb:
                cls._adb_cmd = adb
            else:
                import os as _os
                cls._adb_cmd = _os.path.join("deps", "adb", "adb.exe")
        return cls._adb_cmd
    ...
    def tap(self, x, y, hold_duration):
        ...
        if not IN_CAREER_RUN:
            time.sleep(TAP_EXTRA_DELAY)

    def init_env(self) -> None:
        try:
            result = subprocess.run(
                [self._get_adb(), "-s", self.config.device_name, "shell", "echo", "ok"],
    ...
    def reinit_connection(self):
        try:
            subprocess.run([self._get_adb(), "-s", self.config.device_name, "reconnect"], 
                          capture_output=True, timeout=5)
        except Exception:
            pass
        time.sleep(SCREENCAP_RETRY * 2)
    ...
    def execute_adb_shell(self, cmd, sync):
        cmd_str = self._get_adb() + " -s " + self.config.device_name + " " + cmd
    ...
    def get_devices(self):
        p = os.run_cmd(self._get_adb() + " devices").communicate()
        devices = p[0].decode()
        log.debug(devices)
        return devices
    ...
    def connect_to_device(self):
        p = os.run_cmd(self._get_adb() + " connect " + self.config.device_name).communicate()
        log.debug(p[0].decode())

    # kill_adb_server 停止adb-server
    def kill_adb_server(self):
        p = os.run_cmd(self._get_adb() + " kill-server").communicate()
        log.debug(p[0].decode())
