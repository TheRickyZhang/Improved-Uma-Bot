...
def _configure_device_display(device_id):
    """Ensure device is set to 720x1280 @ 180dpi as required by the bot."""
    adb = _get_adb_path()
    try:
        # Set via wm commands
        subprocess.run([adb, "-s", device_id, "shell", "wm", "size", "720x1280"],
                       capture_output=True, timeout=5)
        subprocess.run([adb, "-s", device_id, "shell", "wm", "density", "180"],
                       capture_output=True, timeout=5)

        # Verify actual framebuffer size via screencap header
        result = subprocess.run([adb, "-s", device_id, "exec-out", "screencap"],
                                capture_output=True, timeout=10)
        if result.returncode == 0 and len(result.stdout) >= 12:
            import struct as _struct
            w, h, _ = _struct.unpack('<III', result.stdout[:12])
            if (w, h) != (720, 1280) and (w, h) != (1280, 720):
                print(f"WARNING: Framebuffer is {w}x{h}, not 720x1280.")
                print("Set Waydroid native resolution with:")
                print("  waydroid prop set persist.waydroid.width 720")
                print("  waydroid prop set persist.waydroid.height 1280")
                print("Then restart: waydroid session stop && waydroid session start")
                sys.exit(1)
            elif (w, h) == (1280, 720):
                print("WARNING: Device is in landscape mode. Rotate to portrait.")
                subprocess.run([adb, "-s", device_id, "shell", "settings", "put", "system",
                               "user_rotation", "0"], capture_output=True, timeout=5)
                time.sleep(1)
            else:
                print(f"Display configured: {w}x{h}")
        time.sleep(0.5)
    except Exception as e:
        print(f"Display config warning: {e}")

def _kill_stale_server(port=8071):
...
def _force_exit(signum, frame):
    """Ensure all threads die on Ctrl+C."""
    try:
        from bot.base.purge import save_scheduler_tasks, save_scheduler_state
        save_scheduler_tasks()
        save_scheduler_state()
...
if __name__ == '__main__':
    ...
    # Try auto-detecting Waydroid IP first
    try:
        ...
        if result.returncode == 0:
            for line in result.stdout.splitlines():
                if "IP address" in line:
                    ...
                    if ip:
                        selected_device = f"{ip}:5555"
                        print(f"Auto-detected Waydroid device: {selected_device}")
                        break
    except Exception:
        pass
    ...
    _configure_device_display(selected_device)
