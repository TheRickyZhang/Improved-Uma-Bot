...
### Supported Emulators
- **Windows**: MuMu Player (recommended)
- **Linux**: Waydroid (requires GApps + libhoudini for ARM translation)
...
sudo iptables -t nat -A POSTROUTING -s 192.168.240.0/24 -o $(ip route get 8.8.8.8 | head -1 | awk '{print $5}') -j MASQUERADE
